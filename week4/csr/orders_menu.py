...
def orders_menu(orders_list, order_status, couriers_list, products_list):
    while True:
        ...
        if orders_menu_input == 0:
            ...
            updated_status_input = int(input("What would you like to update it to?"))
            ...
            for i in orders_list[order_to_update]:
                if i == "courier":
                    for courier_dict in couriers_list:
                        print(
                            f"{couriers_list.index(courier_dict)}-{courier_dict['name']}"
                        )
                elif i == "status":
                    for status in order_status:
                        print(f"{order_status.index(status)}-{status}")
                ...
                if len(update_input) <= 0:
                    ...
                    orders_list[order_to_update][i] = orders_list[order_to_update][i]
                elif i == "status":
                    if len(update_input) <= 0:
                        orders_list[order_to_update][i] = update_input
                    else:
                        orders_list[order_to_update]["status"] = order_status[
                            int(update_input)
                        ]
                else:
